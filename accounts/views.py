        ...
        if form.is_valid():
            ...
            if user is not None:
                try:
                    ...
                    # if user based cart is already present then session cart
                    # mein ko delete karo
                    ...
                    if user_cart:
                        ...
                        for item in user_cart_items:
                            ...
                            existing_variation_sets.append(
                                variation_list)  # compare as sets
                            ids.append(item.id)
                        ...
                        for item in cart_items:
                            ...
                            if current_variation in existing_variation_sets:
                                ...
                                idx = existing_variation_sets.index(
                                    current_variation)
                                # print(idx)
                                item_qty_update = CartItem.objects.get(
                                    id=ids[idx])
...
def profile(request):
    ...
    try:
        user_profile = UserProfile.objects.only(
            'profile_picture').get(user=request.user)
    except Exception as e:
        user_profile = None
...
def order_detail(request, order_id):
    ...
    try:
        ...
        ordered_products = OrderProduct.objects.filter(
            order=order, user=request.user, ordered=True).order_by('updated_at')
