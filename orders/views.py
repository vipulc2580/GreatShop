...
def place_order(request):
    ...
    tax_amount = sum([float(val) for item, value in tax_dict.items() for key, val in value.items()])
...
def payments(request,status,transaction_id,payment_method,order_number):
    ...
    ordered_products=OrderProduct.objects.filter(user=current_user,payment=payment,order=order)
    ...
    send_notification_email(request,current_user,mail_subject,htmlfile,context)
...
def capture_order(request, order_id,order_number):
    ...
    if response.status_code == 201 or response.status_code == 200:
        ...
        return payments(request,status,transaction_id,payment_method,order_number)
...
def order_complete(request):
    ...
    try:
        order=Order.objects.get(order_number=order_number,payment__payment_id=transaction_id,is_ordered=True)
        # print('found order')
        ordered_products=OrderProduct.objects.filter(order=order,payment__payment_id=transaction_id)
